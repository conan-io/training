...
class ConanConditions {
    ...
    static boolean conanLockfilesNeedRebase(CpsScript currentBuild, Map config = null) {
        Map defaultConfig = [
        ...
        return ConanLockfilesNeedRebase.construct(currentBuild, mergedConfig).run()
    }
    ...
    static boolean conanBuildOrderHasSteps(CpsScript currentBuild, Map config = null) {
        Map defaultConfig = [
                "conanRemoteUploadName": "conan-temp",
                "dockerEnvVars"        : [
                        "TRAINING_GIT_CREDS_USR",
        ...
        Map mergedConfig = defaultConfig
        if (config) mergedConfig.putAll(config)
        return ConanBuildOrderHasSteps.construct(currentBuild, mergedConfig).run()
    }
